    ...
    const handleSubmit = (e) => {
        ...
        if (activeTab === 'signup') {
            ...
            if (existingUser && JSON.parse(existingUser).email === formData.email) {
                ...
                localStorage.setItem('rr_user_auth', JSON.stringify({ name: formData.name, email: formData.email }));
            ...
            const existingUser = localStorage.getItem('rr_user_auth');
            if (existingUser && JSON.parse(existingUser).email === formData.email) {
                // Note: In a real app, you would also check the password against a hash.
                localStorage.setItem('rr_is_logged_in', 'true');
                ...
                <div className="hidden lg:block bg-gray-50 p-12">
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
                        ...
                        <div className="flex items-center gap-7 text-lightText">
                           <div className='flex justify-center items-center'>{[0,1,3].map((i)=>(
                           <img src={"https://i.pravatar.cc/150?" + i }alt="user"  className='w-10 border-2 rounded-full -mr-5'/>
                           ))
                           }</div>
